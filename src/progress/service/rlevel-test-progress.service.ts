...
@Injectable()
export class RLevelTestProgressService {
  constructor(
    @InjectRepository(RLevelTestProgress)
    private progressRepo: Repository<RLevelTestProgress>,
  ...
  private async __create(progress: number, user: RUser, levelTest: LevelTest) {
    const entity: RLevelTestProgress = new RLevelTestProgress();
    entity.user = user;
  ...
  private async __update(progress: number, entity: RLevelTestProgress) {
