...
@Injectable()
export class RChinghoService implements OnModuleInit {
  private rareness;
  private title;

  constructor(
    @InjectRepository(Chingho)
    private readonly chinghoRepo: Repository<Chingho>,
  ...
  async create(user: RUser, plateData: RPlateData) {
    const chingho = await this.chinghoRepo.create({
      user: user,
      plateData: plateData,
    });
    return await this.chinghoRepo.save(chingho);
  }
  ...
  async fetchCurrentChingho(userid: number) {
    try {
      return await this.chinghoRepo.findOne({
        where: {
          user: { id: userid },
        },
      });
  ...
  async updateUserChingho(userid: number, dto: UpdateChinghoDto) {
    try {
      await this.chinghoRepo.update({ user: { id: userid } }, dto);
    } catch (e) {
      throw e;
    }
  }
